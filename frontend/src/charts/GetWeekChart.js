...
export default function GetWeekChart({ symbol, options, actualPrice }) {
    ...
    useEffect(() => {
        axios.get(`https://financialmodelingprep.com/api/v3/historical-chart/15min/${symbol}?apikey=638d777cab0c32857e401d69e4a38e52`)
        .then(response => {
    ...
    return (
        <ChartBoxTop>
            <div style={{display: 'flex', justifyContent: 'space-between'}}>
                ...
                {dataSeries[0]>dataSeries[139] ? <h4 style={{margin: '10px 20px 25px', color: dataColor}}>-{Math.round((1-dataSeries[139]/dataSeries[0])*10000)/100}%</h4> : <h4 style={{margin: '10px 20px 25px', color: dataColor}}>+{Math.round((1-dataSeries[139]/dataSeries[0])*(-10000))/100}%</h4>}
            </div>
